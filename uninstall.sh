...
# Removes the alias from your shell profile

echo "Uninstalling Repo Madness..."

# Find which shell profile contains the alias (without grep)
SHELL_PROFILE=""
...
for profile in "${PROFILES_TO_CHECK[@]}"; do
    if [[ -f "$profile" ]]; then
        while IFS= read -r line; do
            case "$line" in
                *alias\ repo=*) SHELL_PROFILE="$profile"; break 2 ;;
            esac
